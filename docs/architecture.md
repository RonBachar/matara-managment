...
Lead ──1:N──► Quote            ליד ← הצעת מחיר ← לקוח (נכנס מהטופס באתר דרך ה־webhook, או ידנית)
```

- `ClientService` נמחק בקסקייד עם הלקוח. `Project` **לא** — לקוח עם פרויקטים לא ניתן למחיקה, בכוונה.
- `Quote.clientId` ו־`Quote.leadId` אופציונליים (`ON DELETE SET NULL`): הצעה נשלחת לליד או ללקוח, וכל ההצעות
  מרוכזות בעמוד `/quotes` (כולל כאלה בלי שיוך). אחרי המרה ההצעה שומרת גם את `leadId`. ה־`slug` (החלק האחרון בכתובת, בלי `.html`) הוא המפתח שמחבר רישום ידני לחתימה.
...
נרשמה). הצעה בלי שיוך משויכת ללקוח היחיד שהאימייל שלו תואם לחותם, ואם אין כזה לליד היחיד שהאימייל או
הטלפון שלו תואמים. הצעה עם ליד ובלי לקוח ממירה את הליד ללקוח (`server/services/leadConversion.ts`, אותה
המרה של "הפוך ללקוח") ומשייכת אליו. `contractUrl` ריק של הלקוח מתמלא בעותק החתום. אידמפוטנטי: קריאה חוזרת לא כותבת כלום.
