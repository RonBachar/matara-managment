...
לפי ה־slug (ונוצרת אם לא נרשמה). הצעה בלי שיוך משויכת ללקוח היחיד שהאימייל שלו תואם לחותם, ואם אין כזה
לליד היחיד שהאימייל או הטלפון (`signerPhone`, אופציונלי) שלו תואמים. הצעה של ליד שנחתמה ממירה את הליד ללקוח
(אותה המרה של "הפוך ללקוח"), וקישור החוזה של הלקוח מתמלא אם הוא ריק. כל ההצעות מרוכזות בעמוד "הצעות מחיר"
(`/quotes`), ומטבלת הלידים אפשר לפתוח הצעה חדשה לליד. קריאה חוזרת עם אותו גוף לא משנה דבר. מחזיר `{ ok, quoteId, clientId }`.
